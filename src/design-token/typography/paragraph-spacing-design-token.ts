...
export class ParagraphSpacingDesignToken extends DesignToken {
  static type: DesignTokenType = 'paragraph-spacing' as const;
  #cssVariablePattern = /var\([^)]+\)/g;

  constructor({ prefix = '' }: { prefix?: string } = {}) {
    super({ type: ParagraphSpacingDesignToken.type, prefix });
  }

  addParagraphSpacing(name: string, fontSize: string): void {
  ...
  override tailwindConfig(): TailwindConfig {
    ...
    this.tokens.forEach((token) => {
      margin[`ps-${token.name}`] = token.css
