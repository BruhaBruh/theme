...
export class BackgroundDesignToken extends DesignToken {
  static type: DesignTokenType = 'bg' as const;
  ...
  constructor(
    ...
    super({ type: BackgroundDesignToken.type, prefix });
    this.#colorDesignToken = colorDesignToken;
  }

  get colorDesignToken(): ColorDesignToken {
    return this.#colorDesignToken;
  }

  addBackgroundColor(name: string, color: string): void {
  ...
  override tailwindConfig(absolute: boolean): TailwindConfig {
    ...
    this.tokens.forEach((token) => {
      backgroundColor[token.name] = token.toTailwindString({
        absolute,
